...
namespace Movies.Server.Infrastructure
{
	...
	public class AdoNetPersistenceProviderOptions
	{
	...
	public static class SiloBuilderExtensions
	{
		...
		public static ISiloBuilder UseAdoNetPersistenceProvider(this ISiloBuilder siloHost, AppSiloBuilderContext context, AdoNetPersistenceProviderOptions grainOptions)
		{
			...
			siloHost
				...
				{
					options.Invariant = grainOptions.Invariant;
					options.ConnectionString = grainOptions.ConnectionString;
					options.UseJsonFormat = grainOptions.UseJson;
				})
				.Configure<ClusterOptions>(options =>
				{
					options.ClusterId = appInfo.ClusterId;
				...
				{
					options.CollectionAge = TimeSpan.FromMinutes(1.5);
				})
				.UseLocalhostClustering(context.SiloOptions.SiloPort, context.SiloOptions.GatewayPort);
